...
const fetchCaseData = async () => {
  try {
    ...
    const [caseRes, evRes, hypRes] = await Promise.all([
      fetch(`http://localhost:5000/api/cases/${route.params.id}`, { headers: h }),
      fetch(`http://localhost:5000/api/cases/${route.params.id}/evidence`, { headers: h }),
      fetch(`http://localhost:5000/api/cases/${route.params.id}/hypotheses`, { headers: h })
    ]);
    ...
    for (const hyp of hypotheses.value) {
       const res = await fetch(`http://localhost:5000/api/hypotheses/${hyp._id}/relationships`, { headers: h });
...
const fetchTimeline = async () => {
  try {
     const res = await fetch(`http://localhost:5000/api/cases/${route.params.id}/timeline`, {
...
const createEvidence = async () => {
   ...
   try {
      const res = await fetch(`http://localhost:5000/api/cases/${route.params.id}/evidence`, {
         method: 'POST',
         headers: headers.value,
...
const verifyEvidence = async (id, state) => {
  try {
     await fetch(`http://localhost:5000/api/evidence/${id}/verify`, {
       method: 'PUT',
       headers: headers.value,
...
const createHypothesis = async () => {
   creatingAction.value = true;
   try {
      const res = await fetch(`http://localhost:5000/api/cases/${route.params.id}/hypotheses`, {
         method: 'POST',
         headers: headers.value,
...
const submitLink = async () => {
   creatingAction.value = true;
   try {
      const res = await fetch(`http://localhost:5000/api/hypotheses/${linkForm.value.hypothesisId}/relationships`, {
         method: 'POST',
         headers: headers.value,
         body: JSON.stringify({
